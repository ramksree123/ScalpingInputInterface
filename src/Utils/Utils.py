...
class Utils:
  ...
  @staticmethod
  def convertEpochToDateTimeString(epochTime):
  ...
  def getWeeklyExpiryDayDate(dateTimeObj = None):
    ...
    #Works when called on Mon-Thursday. If called on Friday,saturday,sunday it returns the past expiry only. 
    #Dont make changes in this as its referenced in many other places. Build new function
    if dateTimeObj == None:
      dateTimeObj = datetime.now()
  ...
  @staticmethod
  def getWeeklyExpiryStartDate(dateTimeObj = None):
    #Created by Sree Ram K
    #Gets the Friday where the next week expiry starts 
    # #if that friday is a holiday, use the thursday as 
    ...
    if dateTimeObj.weekday() <= 4:
      daysToAdd = (4-dateTimeObj.weekday())
    else:
      daysToAdd = 11 - dateTimeObj.weekday()
  ...
  @staticmethod
  def getWeeklyExpiryEndDate(dateTimeObj = None):
    #Created by Sree Ram K
    #Gets the Friday where the next week expiry starts 
    # #if that friday is a holiday, use the thursday as 
    ...
    daysToAdd = 0
    if dateTimeObj.weekday() > 3:
      daysToAdd = 1 * (10-dateTimeObj.weekday())
    else:
      daysToAdd = 3 - dateTimeObj.weekday()
    datetimeExpiryDay = dateTimeObj + timedelta(days=daysToAdd)
  ...
  @staticmethod
  def convertDateStrToDateTimeObj(datetimestring):
    pass
